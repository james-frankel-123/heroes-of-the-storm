...
export async function getDraftData(
  ...
  // Model-feeding stats come from the decayed-aggregate artifact generated by
  // training/production_refresh/refresh.py — the SAME statistics the deployed
  // WP model was trained on (train/serve consistency; exponential decay,
  // half-life 90 days, per the drift paper's recommendation). The artifact
  // deploys atomically with public/models/*.onnx on the standing cadence.
  const tierStats = (draftStatsDecayed as any).tiers[tier] as {
    heroStats: DraftData['heroStats']
    heroMapWinRates: DraftData['heroMapWinRates']
    synergies: DraftData['synergies']
    counters: DraftData['counters']
  }
  const heroStats = tierStats.heroStats
  const heroMapWinRates = tierStats.heroMapWinRates
  const synergies = tierStats.synergies
  const counters = tierStats.counters
