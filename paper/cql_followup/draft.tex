...
\begin{enumerate}
    ...
    \item We show MCTS with an augmented value function achieves the highest overall win probability against behavioral cloning opponents (0.626 avg WP), combining deeper search with a well-calibrated value function.
\end{enumerate}
...
The value function used for MCTS leaf evaluation varies across runs. We train four configurations with different WP models to measure how value function quality affects MCTS policy quality.
...
\begin{table}[t]
\centering
\caption{MCTS training runs. Win rate and average WP measured against behavioral cloning (GD) opponents.}
\label{tab:mcts}
\begin{tabular}{llrcc}
\toprule
...
\caption{MCTS learning curves across four training runs. Left: win rate against behavioral cloning opponents. Right: average win probability of terminal draft states. The augmented value function (Run G, green) achieves the highest average WP, confirming that synthetic OOD augmentation benefits MCTS training.}
\label{fig:mcts}
\end{figure*}

Table~\ref{tab:mcts} and Fig.~\ref{fig:mcts} show the results. Switching from the base WP model to the enriched model produces an 11.5pp win rate improvement (78.0\% $\to$ 89.5\%) against behavioral cloning opponents. The augmented WP model (Run G) achieves 0.626 average WP at 226K episodes, already higher than any completed run's final WP (0.602), confirming that synthetic augmentation benefits MCTS training through better OOD calibration.
...
The comp\_wr feature alone (Run F) does not improve over the enriched model (Run E) in MCTS, likely because the MCTS policy head learns to avoid degenerate compositions through self-play, making the explicit composition penalty redundant. The augmented model's benefit comes from better calibration across the full state space.

\subsection{Search Depth and Value Function Calibration}

The 26.5\% residual degenerate rate from greedy search is partially a depth-1 limitation. MCTS with 200 simulations provides multi-step lookahead, allowing the agent to plan for future healer availability rather than evaluating one pick at a time. The augmented WP model's higher average WP in MCTS (0.626 vs.\ 0.602) suggests the augmentation helps even with deeper search. Rich evaluation metrics (counter responsiveness, synergy exploitation, hero diversity) for the MCTS policy are not yet available and remain an important direction for confirming whether MCTS achieves context-awareness alongside the safety improvements demonstrated by its higher WP scores.

This subsection also explains why comp\_wr helps greedy search but not MCTS (Run F $\approx$ Run E in Table~\ref{tab:mcts}). MCTS addresses the greedy search horizon problem directly: the tree search can explore sequences where a healer is picked at step 5 even though step 3's greedy evaluation would prefer a damage dealer. The policy head, trained through self-play, provides implicit composition regularization; the network learns to avoid degenerate compositions from the self-play training signal, even without explicit composition features in the value function. Comp\_wr is therefore redundant for MCTS but critical for greedy search, because MCTS learns its own implicit composition policy through experience while greedy search relies entirely on the value function for every decision.
...
Table~\ref{tab:throughput} shows the progression of optimizations. The fused kernel with pipelining achieves 48 episodes/second per GPU, a 40$\times$ improvement over the initial Python implementation and sufficient to complete 300K-episode training runs in under two hours per GPU.
...
+ ring buffer + pipelining & \textbf{48} & Near hardware ceiling \\
\bottomrule
\end{tabular}
\end{table}
...
CQL's apparent success (9\% degenerate with naive features) is misleading: it collapses to behavioral cloning as shown by negative counter/synergy deltas, half the hero diversity, and 62\% GD agreement. Adding enriched features to CQL does not recover context-awareness. MCTS with the augmented value function achieves the highest overall win probability against behavioral cloning opponents (0.626 avg WP, 92.5\% win rate), combining deeper search with a well-calibrated value function, though rich evaluation metrics for the MCTS policy remain to be measured.
...
\textit{Limitations.} Single game domain, fixed GD opponent model, greedy search for most comparisons. The CQL collapse may be specific to domains with concentrated behavior policies. MCTS rich evaluation metrics are not yet available.

\textit{Future work.} Full rich evaluation for MCTS policies (counter/synergy/diversity), offline RL methods that preserve action diversity while maintaining conservatism, and testing the safety/context-awareness decomposition in other combinatorial domains (team sports drafts, card game deck building, portfolio selection).
...
``When Similar Accuracy Produces Different Policies: Value Function Features Matter More Than Aggregate Metrics,'' in \textit{Proc. IEEE Conf. Games (CoG)}, 2026.
