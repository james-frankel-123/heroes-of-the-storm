...
Adding the 86 enriched features to CQL's input (375 total dims) improves composition safety (3.0\% degen at $\alpha = 2.0$) but does not produce context-awareness. Counter responsiveness remains negative ($-$0.10 to $-$0.12), synergy remains near-zero ($-$0.04 to $-$0.15), and hero diversity stays at 38 heroes with 72--74\% top-10 concentration. GD similarity actually \textit{increases} with enriched features (63--70\% vs.\ 57--62\% for naive CQL), confirming that the enriched features help CQL be more precise about what the behavior policy's distribution looks like rather than enabling it to deviate from that distribution in productive ways. CQL's pessimism dominates regardless of input features, preventing matchup-dependent decisions.
