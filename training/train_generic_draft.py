...
def replay_to_training_samples(replay: dict) -> list[tuple[np.ndarray, int, np.ndarray]]:
    ...
    for step in draft_order:
        ...
        if step_type == 0:  # ban
            ...
            if slot <= 4 or slot == 1:
                # Heuristic: even slots or slot 1 = team 0 picks
                # More reliable: track by team assignment in the draft
...
def train():
    ...
    model = GenericDraftModel().to(device)
    ...
    for epoch in range(200):
        ...
        if avg_test_loss < best_test_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(os.path.dirname(__file__), "generic_draft.pt"))
        else:
            patience_counter += 1
    ...
    # Export to ONNX
    model.load_state_dict(torch.load(os.path.join(os.path.dirname(__file__), "generic_draft.pt"),
                                     weights_only=True))
    model.eval()
    dummy_x = torch.randn(1, INPUT_DIM)
    dummy_mask = torch.ones(1, NUM_HEROES)
    onnx_path = os.path.join(os.path.dirname(__file__), "generic_draft.onnx")
    torch.onnx.export(
